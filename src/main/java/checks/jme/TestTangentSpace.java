...
public class TestTangentSpace extends SimpleApplication {
    ...
    public static final Class<GrayF32> IMAGE_TYPE = GrayF32.class;
    private ImagePanel gui = new ImagePanel();
    private final ImageProcessor imageProcessor = new DetectDirsProcessor<>(IMAGE_TYPE);
    private SimpleImageSequence sequence = new CameraImageSequence(ImageType.single(IMAGE_TYPE));



    int count = 0;
    LinkedList<Vector3f> steps = new LinkedList<>();
    {
...
//        gui.setPreferredSize(new Dimension( (int)(500), (int) (500)));
//        ShowImages.showWindow(gui,"KTL Tracker", true);
    ...
    public void update() {
        ...
        cam.setLocation(cam.getLocation().add(steps.get(count% steps.size())));
        count ++;
//        if (count%100 == 0) {
            System.out.println("Path of store " + JmeSystem.getStorageFolder().getAbsolutePath());
...
//        final ImageBase next = sequence.next();
//        imageProcessor.tracking((ImageGray) next);
//        final BufferedImage image = imageProcessor.processImage((BufferedImage) sequence.getGuiImage(), sequence.getFrameNumber());
//        updateGUI(image);
